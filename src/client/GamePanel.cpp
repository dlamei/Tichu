...
namespace GamePanel {
    ...
    bool has_me_finished(const Data &data) {
        auto me = get_me(data);
        if (me && (*me)->get_is_finished()) return true;
        else return false;
    }
    ...
    bool is_my_turn(const Data &data) {
        ...
        if (data.game_state.get_game_phase() == GamePhase::SELECTING) return false;
