...
//          _player_name(name)
//{ }
join_game_request::join_game_request(const std::string &name)
          : _player_name(name)
{ }
...
//join_game_request::join_game_request(client_request::base_properties props, std::string player_name) :
...
join_game_request join_game_request::from_json(const rapidjson::Value& json) {
    if (json.HasMember("player_name")) {
        return join_game_request { json["player_name"].GetString()};
    } else {
