...
template <class T>
class serializable_value : public reactive_object {
...
protected:
    T _value;
    serializable_value(base_params params, T val) : reactive_object(params), _value(val) { }

public:
    explicit serializable_value(std::string var_name, T val) : reactive_object(var_name), _value(val) { }
...
// serializable interface
    ...
    static serializable_value<T>* from_json(const rapidjson::Value& json) {
        if (json.HasMember("value")) {
            ...
            return new serializable_value<T>(params, val);
        }
        return nullptr;
    }
};
